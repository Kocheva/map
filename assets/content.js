  ...
  return el(
    ...
    el(
      ...
      el(
        ...
        el(
          "span",
          { className: "grany-link" },
          external(SITE_LINKS.grany, "Центр ГРАНИ"),
        ),
      ),
    ),
    el(
...
export function Methodology({ data }) {
  ...
  return el(
    ...
    section(
      ...
      para(
        `В текущей версии — ${data.people.length} персон. Отдельная группа — ${leaders} руководителя проектов — победителей конкурсов Фонда Тимченко: в реестре представлены ${data.fund_projects.length} проектов и ${data.fund_contest_groups.length} конкурсов. В карточках сохранены названия отдельных годов и этапов поддержки.`,
      ),
      para(
    ...
    section(
      ...
      para(
        "В фильтре годы и этапы поддержки объединены по названию конкурса. Период охватывает годы, указанные в названиях этапов и годах конкурсов текущего реестра. Число в скобках — общее количество записей о проектах этого конкурса, включая этапы поддержки; оно не меняется при выборе других фильтров. Исходные названия, годы и этапы сохранены в карточках проектов. В открытой карточке видны все проекты человека, включая проекты других конкурсов.",
      ),
      para(
